...
class TestSleepService:
    ...
    def test_get_sleep_stats(self, engine: Engine) -> None:
        ...
        sleep_stats: List[SleepStatsDTO] = SleepService(engine).get_sleep_stats(date(2025, 1, 1), date(2025, 4, 6))
        # Plot sleep duration using matplotlib
        import matplotlib.pyplot as plt
        import numpy as np

        # Extract sleep durations
        sime_day_tuple = [(stat.date, ((stat.start_time) % (24 * 3600)) / 3600.0 + 8.0, ((stat.end_time - 6 * 3600) % (24 * 3600)) / 3600.0 + 14.0, stat.duration) for stat in sleep_stats]
        # Filtering
        sime_day_tuple = [(date, start_hour, end_hour, duration) for date, start_hour, end_hour, duration in sime_day_tuple if start_hour > 20.0 and end_hour > 24.0 + 3.0]
        # Sort by date
        ...
        duration_moving_avg = np.convolve(durations, np.ones(window_size)/window_size, mode='valid')
